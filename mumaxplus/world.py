...
class World:
    ...
    def minimize(self, tol=1e-6, nsamples=10, tol_el=1e-6, nsamples_el=10, stepsize_el=1e-30, stepsize_el_fallback=1e-30,
        ...
        when starting from a high energy state. If any magnet in the world has elastodynamics enabled,
        this simultaneously minimizes the elastic displacement towards mechanical 
        equilibrium (minimal effective body force), using its own independent convergence
        check. If elastodynamics is not enabled for any magnet, the elastic parameters are ignored.
        ...
        initial_rot : int (default=0)
            Perform an initial full rotation prior to minimizing. Useful if there is an existing large angle rigid-body
            rotation in the displacement, since per-step projection is linearized (small angle approximation).
