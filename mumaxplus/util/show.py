...
def hsl_to_rgb(H, S, L):
    ...
    # G = m + C for 1<=Hp<3
    G = m + _np.select(
        [(Hp < 1) | ((3 <= Hp) & (Hp < 4)), (1 <= Hp) & (Hp < 3)], [X, C], 0.0
    )
    # B = m + X for 2<=Hp<3 or 5<=Hp<6
    # B = m + C for 3<=Hp<5
    B = m + _np.select(
        [((2 <= Hp) & (Hp < 3)) | (5 <= Hp), (3 <= Hp) & (Hp < 5)], [X, C], 0.0
    )

    # clip rgb values to be in [0,1]
    R, G, B = _np.clip(R, 0.0, 1.0), _np.clip(G, 0.0, 1.0), _np.clip(B, 0.0, 1.0)
...
def get_rgb(
    field_quantity: _mxp.FieldQuantity | _np.ndarray,
    ...
    if layer is not None and OoP_axis_idx is not None:
        if (ndim := len(field_quantity.shape)) != 4:
            raise IndexError(
                f"Can not take layer index {layer} of axis "
    ...
    if OoP_axis_idx not in [0, 1, 2, None]:
        raise IndexError(
            f"OoP_axis_index should be 0, 1, 2 or None, not {OoP_axis_idx}."
        )
...
def _get_axis_components(
    out_of_plane_axis: Literal["x", "y", "z"]
) -> tuple[int, int, int]:
    """Translate out of plane axis string to a right handed coordinate system
    ...
    raise ValueError(
        f"Unknown axis '{out_of_plane_axis}', use 'x', 'y' or 'z' instead."
    )


def slice_field_right_handed(
    field: _np.ndarray, OoP_axis_idx: Literal[0, 1, 2] = 2, layer: int = 0
) -> _np.ndarray:
...
def _get_colorbar_verticality(colorbar_kwargs: dict) -> bool:
    ...
    if "location" in colorbar_kwargs.keys():  # location has priority
        if (
            colorbar_kwargs["location"] == "top"
            or colorbar_kwargs["location"] == "bottom"
        ):
            return False
    elif (
        "orientation" in colorbar_kwargs.keys()
        and colorbar_kwargs["orientation"] == "horizontal"
    ):
        return False
...
def _get_length_fraction_inside(o_i: int, n_i: int, r: float) -> float:
    """Return the fraction of the side length of the old small cell that is
...
def downsample(
    field: _np.ndarray, new_shape: tuple, intrinsic: bool = True
) -> _np.ndarray:
    ...
    if ndim != len(new_shape):
        raise ValueError(
            "New shape does not have same number of dimensions as old shape."
        )
    ...
    for idx_new in _np.ndindex(new_shape):  # loop over all new cells
        ...
        old_ranges = [
            range(int(i_new * ratio), _math.ceil((i_new + 1) * ratio))
        ...
        for idx_old in product(*old_ranges):
            ...
            frac = 1.0
            for i in range(ndim):
                frac *= _get_length_fraction_inside(
                    idx_old[i], idx_new[i], on_shape_ratio[i]
                )
    ...
    if intrinsic:
        new_field /= _np.prod(on_shape_ratio)
...
# ----- end of downsample -----
# ----- SI units -----
# This code is copied from the Hotspice code written by Jonathan Maes
# and slightly tweaked.
# https://github.com/bvwaeyen/Hotspice/blob/main/hotspice/utils.py#L129

SIprefix_to_magnitude = {
    "q": -30,
    "r": -27,
    "y": -24,
    "z": -21,
...
def SIprefix_to_mul(unit) -> float:
    """Return multiplier given an SI prefix."""
...
def appropriate_SIprefix(
    n: float | _np.ndarray, unit_prefix="", only_thousands=True
) -> tuple[float, str]:
    """Convert `n` with old SI prefix `unit_prefix` to a reasonable number with
    a new SI prefix.
    ...
    """
    # If `n` is an array, use the average of absolute values as representative
    # of the scale
    ...
    nearest_magnitude = (
        round(_np.log10(abs(value))) if value != 0 else 0
    ) + offset_magnitude
    # Make sure it is in the known range
    nearest_magnitude = _np.clip(
        nearest_magnitude,
    ...
    if only_thousands:
        supported_magnitudes = [mag for mag in supported_magnitudes if (mag % 3) == 0]
    ...
    return (
        n / 10 ** (used_magnitude - offset_magnitude),
        magnitude_to_SIprefix[used_magnitude],
    )
...
class UnitScalarFormatter(_matplotlib.ticker.ScalarFormatter):
    ...
    def format_data(self, value):
        """Return the formatted string for a tick value, scaled by SImultiplier."""
        # TODO: figure out the use of this function
        return super().format_data(value / self.SImultiplier)

    def format_data_short(self, value):
        """Exactly the same implementation as with ScalarFormatter. # noqa
...
class _Plotter:
    """Intended for organizing the data and parameters of
    ...
    def __init__(
        ...
        if not is_quantity and not isinstance(field_quantity, _np.ndarray):
            raise TypeError(
                "The first argument should be a FieldQuantity or an ndarray."
            )
        ...
        if is_quantity and field is not None and field.shape != field_quantity.shape:
            raise ValueError(
                "The field_quantity and field need to have the same shape."
            )
        ...
        if component is not None:
            if component >= self.ncomp:
                raise IndexError(
                    f"Component index {component} out of range, "
                    + f"must be less than {self.ncomp}."
                )
        elif (self.ncomp != 1) and (self.ncomp != 3):
            raise ValueError(
                f"Cannot plot field quantity with {self.ncomp} "
        ...
        self.hor_axis_idx, self.vert_axis_idx, self.OoP_axis_idx = _get_axis_components(
            out_of_plane_axis
        )
        ...
        if geometry is not None:
            if geometry.shape == field_quantity.shape[1:]:
                self.geom_2D = slice_field_right_handed(
                    geometry, self.OoP_axis_idx, self.layer
                )
            else:
                raise ValueError(
            ...
            self.geom_2D = slice_field_right_handed(
                self.quantity._impl.system.geometry, self.OoP_axis_idx, self.layer
            )
        ...
        im_extent = _quantity_2D_extent(
            field_quantity, self.hor_axis_idx, self.vert_axis_idx
        )
        self.imshow_kwargs.setdefault("extent", im_extent)
        self.imshow_kwargs.setdefault("origin", "lower")
        # vector image or scalar image? vector if field with 3 components,
        # but none selected
        self.vector_image_bool = (self.ncomp == 3) and (self.comp is None)

        # imshow symmetric clim
        if (
            "vmin" not in self.imshow_kwargs.keys()
            and "vmax" not in self.imshow_kwargs.keys()
        ):
        ...
        if self.imshow_symmetric_clim:
            self.imshow_kwargs.setdefault("cmap", "bwr")
        ...
        if self.enable_quiver:
            ...
            if (
                "vmin" not in self.quiver_kwargs.keys()
                and "vmax" not in self.quiver_kwargs.keys()
            ):
            ...
            if "cmap" in self.quiver_kwargs.keys():
                if quiver_cmap is None:  # use quiver_cmap, not cmap kwarg
                    ...
                    _warnings.warn(
                        "The quiver colormap is provided twice, using `quiver_cmap`."
                    )
    ...
    def plot_image(self):
        ...
        if self.vector_image_bool:  # vector field
            im_rgba = get_rgba(
                self.field_2D, geometry=self.geom_2D, OoP_axis_idx=None, layer=None
            )
            ...
            if (
                "vmin" not in self.imshow_kwargs.keys()
                and "vmax" not in self.imshow_kwargs.keys()
                and self.imshow_symmetric_clim
            ):
            ...
            # cbar
            # Name only the component if relevant. Let title display quantity name,
            # unless empty
            if self.ncomp > 1:
    ...
    def plot_quiver(self):
        ...
        if (
            "scale" not in self.quiver_kwargs
            and "scale_units" not in self.quiver_kwargs
        ):
        ...
        if isinstance(self.quiver_cmap, str) and "hsl" in self.quiver_cmap.lower():
        ...
        elif self.quiver_cmap is None:  # uniform color
            ...
            self.quiver = self.ax.quiver(
                ...
                V,
                sampled_field_OoP,
                cmap=self.quiver_cmap,
                **self.quiver_kwargs,
            )
            self.add_cbar(
                self.quiver, name=f"Out-of-plane {self.out_of_plane_axis}-component"
            )

    def add_cbar(self, cp, name: str = ""):
        """Add a colorbar associated with the given plot next to `self.ax`, if
        ...
        # make cax so cbar scales with ax height, if no sizes/positions are
        # user-provided
        if not any(
            [
                k in cbar_kwargs.keys()
                for k in [
                    "ax",
                    "cax",
                    "location",
            ...
            cbar_kwargs["cax"] = self.divider.append_axes(
                position="right", size="5%", pad=pad
            )
    ...
    def replace_get_cursor_data(self):
        """Replace the `get_cursor_data` method of the AxesImage artist `self.im`
    ...
    def replace_format_cursor_data(self):
        """Replace the `format_cursor_data` method of the AxesImage artist
    ...
    def dress_axes(
        self, max_width_over_height_ratio=6.0, max_height_over_width_ratio=3.0
    ):
        ...
        if self.title is None:  # make default title
            ...
            component = ""
            if (
                not self.vector_image_bool
                and not self.enable_colorbar
                and self.ncomp > 1
            ):
            ...
            # combine into title
            title = (
                component + " of " + name if component and name else component + name
            )
...
def plot_field(
    field_quantity: _mxp.FieldQuantity | _np.ndarray,
    out_of_plane_axis: Literal["x", "y", "z"] = "z",
    layer: int = 0,
    ...
    imshow_symmetric_clim: bool = False,
    imshow_kwargs: dict = {},
    enable_colorbar: bool = True,
    colorbar_kwargs: dict = {},
    enable_quiver: bool = None,
    arrow_size: float = 8.0,
    quiver_cmap: Optional[str] = None,
    quiver_symmetric_clim: bool = True,
    quiver_kwargs: dict = {},
) -> Axes:
    ...
    ----------
    field_quantity : mumaxplus.FieldQuantity or numpy.ndarray
        The field_quantity needs to have 4 dimensions with the shape
        (ncomp, nx, ny, nz). Additional dressing of the Axes is done if given a
        mumaxplus.FieldQuantity.

    out_of_plane_axis : string, default="z"
        The axis pointing out of plane: "x", "y" or "z". The other two are plotted
...
def inspect_field(
    ...
    left, right, bottom, top = _quantity_2D_extent(
        field_quantity, hor_axis_idx, vert_axis_idx
    )
    hw_ratio = (top - bottom) / (right - left)
    ...
    if nrows is None and ncols is None:  # find suitable arrangement
        ...
        nrows, ncols = (n_l, n_s) if hw_ratio < 1.0 else (n_s, n_l)
    ...
    fig, axs = _plt.subplots(
        nrows=nrows, ncols=ncols, sharex="all", sharey="all", figsize=figsize
    )
    try:
    ...
    if suptitle is None:  # make default suptitle
        ...
        # combine
        suptitle = (
            qname + " in " + layer_str if qname and layer_str else qname + layer_str
        )
    ...
    if enable_shared_colorbar:
        ...
        for key, value in [("vmin", vmin), ("vmax", vmax)]:
            if key not in _imshow_kwargs.keys() or (
                key in _imshow_kwargs and _imshow_kwargs[key] is None
            ):
...
def show_magnet_geometry(magnet):
    ...
    # Import here to not make this a required dependency of mumaxplus
    import pyvista as _pv

    geom = magnet.geometry

    # [::-1] for [x,y,z] not [z,y,x] and +1 for cells, not points
    image_data = _pv.ImageData(
        dimensions=_np.array(geom.shape[::-1]) + 1,
...
def show_field_3D(quantity, cmap="HSL", enable_quiver=True, symmetric_clim=True):
    ...
    if quantity.ncomp != 3:
        raise ValueError(
            "Can not create a vector field image because the field"
    ...
    image_data = _pv.ImageData(
        dimensions=_np.asarray(shape) + 1,  # cells, not points
        spacing=cell_size,
        origin=_np.asarray(quantity._impl.system.origin) - 0.5 * _np.asarray(cell_size),
    )
    ...
    if enable_quiver:  # use cones to display direction
        ...
        if "hsl" in cmap.lower():  # Use the HSL colorscheme
            ...
            rgb = get_rgb(
                threshed["field"].T, OoP_axis_idx=None, layer=None, geometry=None
            )
            ...
            quiver.point_data["z-component"] = _np.repeat(
                threshed["field"][:, 2], cres + 1, axis=0
            )
            clim = None
            if symmetric_clim:
                ...
                clim = (-vmax, vmax)
            plotter.add_mesh(
                quiver, scalars="z-component", cmap=cmap, clim=clim, lighting=False
            )
    else:  # use colored voxels
        if "hsl" in cmap.lower():  # Use the HSL colorscheme
            # don't need to set opacity for geometry, threshold did this
            threshed.cell_data["rgb"] = get_rgb(
                threshed["field"].T, OoP_axis_idx=None, layer=None, geometry=None
            )
            ...
            if symmetric_clim:
                ...
                clim = (-vmax, vmax)
            plotter.add_mesh(
                threshed, scalars="z-component", cmap=cmap, clim=clim, lighting=False
            )
