...
class TestNcAfmExchange:
    ...
    def test_homo_exchange(self):
        ...
        for i in range(3):
            ...
            sub2, sub3 = (
                magnet.sublattices[(i + 1) % 3],
                magnet.sublattices[(i + 2) % 3],
            )
    ...
    def test_exchange_spiral(self):
        ...
        while (max_angle := magnet.sub1.max_angle()) < (20 * np.pi / 180):  # to 20°
            ...
            E_inhomo_mumax.append(
                magnet.sub1.inhomogeneous_exchange_energy() / (Km(msat) * V)
            )
            E_fm_theory.append(aex * kx**2 / Km(msat))
            E_inhomo_theory.append(-0.5 * ncafmex_nn * kx * kx / Km(msat))

            kx += kx_step
            magnet.sub1.magnetization = (
                np.cos(kx * X),
                np.sin(kx * X),
                np.zeros(shape=X.shape),
            )
            magnet.sub2.magnetization = (
                -np.cos(kx * X),
                -np.sin(kx * X),
                np.zeros(shape=X.shape),
            )
        ...
        error = [
            np.abs(a - e) / a for (e, a) in zip(E_inhomo_mumax[1:], E_inhomo_theory[1:])
        ]
        assert max(error) < 1e-2
    ...
    def test_exchange_spiral(self):
        ...
        while phi < np.pi:
            ...
            E_homo_mumax.append(
                magnet.sub1.homogeneous_exchange_energy() / (Km(msat) * V)
            )
            E_inhomo_mumax.append(
                magnet.sub1.inhomogeneous_exchange_energy() / (Km(msat) * V)
            )
        ...
        # semi relative error
        error = [
            np.abs(a - e) / np.max(np.abs(E_homo_theory))
            for (e, a) in zip(E_homo_mumax[1:], E_homo_theory[1:])
        ]
        assert max(error) < 1e-2
        error = [
            np.abs(a - e) / np.max(np.abs(E_homo_theory))
            for (e, a) in zip(E_inhomo_mumax[1:], E_inhomo_theory[1:])
        ]
        assert max(error) < 1e-2
