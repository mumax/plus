...
def run_with_precision(precision: Literal["SINGLE", "DOUBLE"]):
    FP_PRECISION_CPP = {"SINGLE": 1, "DOUBLE": 2}.get(precision)
    PREC_TEST = {"SINGLE": "> 1e-4", "DOUBLE": "< 2e-6"}.get(precision)
    ...
    subprocess.check_output(
        [sys.executable, "-c", code, "--mumaxplus-fp-precision", precision],
        cwd=Path(__file__).parent,
    )
    subprocess.check_output(
        [sys.executable, "-c", code],
        cwd=Path(__file__).parent,
        env=os.environ | {"MUMAXPLUS_FP_PRECISION": precision},
    )
