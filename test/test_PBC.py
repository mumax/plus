...
def simulate():
    ...
    magnet = Ferromagnet(
        world,
        Grid((nx * 5, ny * 5, 1)),
        geometry=r.repeat((0, 0, None), ((nx) * c, (ny) * c, None)),
    )
    magnet.msat = 1000e3
    magnet.aex = 10e-12
    magnet.alpha = 1
...
def simulate_PBC(corners=False):
    ...
    # === mumax⁺ PBC ===
    world_PBC = World(
        cellsize=(c, c, c), mastergrid=Grid((nx, ny, 0)), pbc_repetitions=(2, 2, 0)
    )
    r_PBC = shape.Rectangle(nx / 2 * c, ny / 2 * c)
    r_PBC.translate((nx / 2) * c, (ny / 2) * c, 0)

    if not corners:
        magnet_PBC = Ferromagnet(
            world_PBC,
            Grid((nx, ny, 1)),
            geometry=r_PBC.repeat((0, 0, None), ((nx) * c, (ny) * c, None)),
        )
    else:
        magnet_PBC = Ferromagnet(
            world_PBC,
            Grid((nx, ny, 1)),
            geometry=r_PBC.repeat((0, 0, None), ((nx) * c, (ny) * c, None)).translate(
                nx / 2 * c, ny / 2 * c, 0
            ),
        )
    ...
    magnet_PBC.alpha = 1
    magnet_PBC.magnetization = (1.0, 0.1, 0.01)
...
class TestPBC:
    def test_PBC_demag(self):
        ...
        err = max_relative_error(
            result=np.array(PBC_demag)[
                :,
                :,
                int(ny / 4) + 1 : int(3 * ny / 4) + 1,
                int(nx / 4) + 1 : int(3 * nx / 4) + 1,
            ],
            wanted=np.array(demag)[
                :,
                :,
                int(ny * 5 / 2 - ny / 4) + 1 : int(ny * 5 / 2 + ny / 4) + 1,
                int(nx * 5 / 2 - nx / 4) + 1 : int(nx * 5 / 2 + nx / 4) + 1,
            ],
        )
        assert err < RTOL
    ...
    def test_PBC_run_center(self):
        ...
        err = max_relative_error(
            result=av_mag, wanted=np.array([0.89947968, 0.23352228, -0.00010287])
        )
        assert err < RTOL
