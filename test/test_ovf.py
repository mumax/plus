...
class TestOVF:
    def setup_class(self):
        ...
        self.ovf_aex = pyovf.read(
            self.magnet.aex.name.replace(":", "_") + f"_{0:06d}.ovf"
        )
        self.ovf_strain = pyovf.read(
            self.magnet.strain_tensor.name.replace(":", "_") + f"_{0:06d}.ovf"
        )
    ...
    def test_6D(self):
        ...
        self.magnet.elastic_displacement = (
            np.random.rand(3, nz, ny, nx) * 1e-14
        )  # Change strain_tensor
    ...
    def test_size(self):
        cx_ovf_m, cy_ovf_m, cz_ovf_m = (
            self.ovf_m.xstepsize,
            self.ovf_m.ystepsize,
            self.ovf_m.zstepsize,
        )
        cx_ovf_aex, cy_ovf_aex, cz_ovf_aex = (
            self.ovf_aex.xstepsize,
            self.ovf_aex.ystepsize,
            self.ovf_aex.zstepsize,
        )
        cx_ovf_strain, cy_ovf_strain, cz_ovf_strain = (
            self.ovf_strain.xstepsize,
            self.ovf_strain.ystepsize,
            self.ovf_strain.zstepsize,
        )
        cx, cy, cz = self.world.cellsize
        assert (
            max_relative_error(cx_ovf_m, cx) < RTOL
            and max_relative_error(cy_ovf_m, cy) < RTOL
            and max_relative_error(cz_ovf_m, cz) < RTOL
        )
        assert (
            max_relative_error(cx_ovf_aex, cx) < RTOL
            and max_relative_error(cy_ovf_aex, cy) < RTOL
            and max_relative_error(cz_ovf_aex, cz) < RTOL
        )
        assert (
            max_relative_error(cx_ovf_strain, cx) < RTOL
            and max_relative_error(cy_ovf_strain, cy) < RTOL
            and max_relative_error(cz_ovf_strain, cz) < RTOL
        )

    def test_grid(self):
        nx_ovf_m, ny_ovf_m, nz_ovf_m = (
            self.ovf_m.xnodes,
            self.ovf_m.ynodes,
            self.ovf_m.znodes,
        )
        nx_ovf_aex, ny_ovf_aex, nz_ovf_aex = (
            self.ovf_aex.xnodes,
            self.ovf_aex.ynodes,
            self.ovf_aex.znodes,
        )
        nx_ovf_strain, ny_ovf_strain, nz_ovf_strain = (
            self.ovf_strain.xnodes,
            self.ovf_strain.ynodes,
            self.ovf_strain.znodes,
        )
        nx, ny, nz = self.magnet.grid.size
