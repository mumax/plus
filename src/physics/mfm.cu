...
#pragma unroll
        for (int i = -1; i <= 1; i++) {
          ...
          real3 R = {x0 - x, y0 - y, z0 - z + (lift + i * delta)};
          real r = norm(R);
...
MFM::MFM(Magnet* magnet, const Grid grid, std::string name)
    : name_(name),
  ...
  if (system_->grid().size().z > 1) {
    throw std::invalid_argument(
        "MFM should scan a 2D surface. Reduce "
        "the number of z-cells to 1.");
  }

  if (magnet->world()->pbcRepetitions().z > 0) {
    throw std::invalid_argument(
        "Cannot take MFM picture when periodic "
        "boundaries are enabled in the z-direction.");
  }
  magnets_[magnet->name()] = magnet;
}

MFM::MFM(const MumaxWorld* world, const Grid grid, std::string name)
  ...
  if (system_->grid().size().z > 1) {
    throw std::invalid_argument(
        "MFM should scan a 2D surface. Reduce the "
        "number of cells in the z direction to 1.");
  }

  if (world->pbcRepetitions().z > 0) {
    throw std::invalid_argument(
        "MFM picture cannot be taken if PBC are "
...
Field MFM::eval() const {
  ...
  for (const auto& pair : magnets_) {
    ...
    if (const Ferromagnet* mag = magnet->asFM()) {
      ...
      throw std::invalid_argument(
          "Cannot calculate MFM of instance which "
          "is no Ferromagnet or HostMagnet.");
    }

    cudaLaunch(ncells, k_magneticForceMicroscopy, mfm.cu(), magnetization.cu(),
               magnetNCells, mastergrid, pbcRepetitions, lift, tipsize, V,
               crashed.get());

    bool crashedResult;
    checkCudaError(cudaMemcpyAsync(&crashedResult, crashed.get(), sizeof(bool),
                                   cudaMemcpyDeviceToHost, getCudaStream()));

    if (crashedResult) {
      throw std::invalid_argument(
          "Tip crashed into the sample. increase "
