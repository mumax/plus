...
StrayFieldKernel::~StrayFieldKernel() { delete kernel_; }
...
__global__ void k_strayFieldKernel(CuField kernel, real3 cellsize) {
  ...
  if (!kernel.cellInGrid(idx)) return;
...
Grid StrayFieldKernel::grid() const { return kernelSystem_->grid(); }
real3 StrayFieldKernel::cellsize() const { return cellsize_; }

const Field& StrayFieldKernel::field() const { return *kernel_; }
...
Grid StrayFieldKernel::kernelGrid(Grid dst, Grid src) {
  ...
  int3 padding{0, 0, 0};
  if (size.x > 5 && size.x % 2 == 1) padding.x = 1;
  if (size.y > 5 && size.y % 2 == 1) padding.y = 1;
  if (size.z > 5 && size.z % 2 == 1) padding.z = 1;
