...
Field evalNeelvector(const HostMagnet* magnet) {
  // Calculate a weighted Neel vector (dimensionless) to account for
  // ferrimagnets
  if (magnet->sublattices().size() != 2)
    throw std::runtime_error(
        "Cannot compute the Néel vector if the magnet has no two sublattices");

  Field neel(magnet->system(), 3);

  if (magnet->sublattices()[0]->msat.assuredZero() &&
      magnet->sublattices()[1]->msat.assuredZero()) {
    neel.makeZero();
    return neel;
  }
