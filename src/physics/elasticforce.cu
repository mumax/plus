...
// Divergence of stress with central five-point stencil in bulk material.
// Lower order accuracy (three-point stencil) central difference is used in bulk
// 1 cell away from boundary.
// At the boundary, traction-free bondary conditions are implemented, using a
...
Field evalElasticForce(const Magnet* magnet) {
  ...
  CuField stressTensor = magnet->elasticStressTensor()->field().cu();
  real3 w = 1. / magnet->cellsize();
  Grid mastergrid = magnet->world()->mastergrid();

  cudaLaunch(ncells, k_elasticForce, fField.cu(), stressTensor, w, mastergrid);
