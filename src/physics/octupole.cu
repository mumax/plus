...
__device__ real3 rotate_120(real3 m, real3 ref) {
  ...
  return -0.5 * m +
         cross(k, m) * 0.86602540378 * dir;  // Use sqrt(3) / 2 = 0.86602540378
}
...
__global__ void k_octupolevector(CuField octupole,
  ...
  octupole.setVectorInCell(
      idx, (m1 * ms1 + m2_rot * ms2 + m3_rot * ms3) / (ms1 + ms2 + ms3));
}

Field evalOctupoleVector(const NcAfm* magnet) {
  // Calculate a weighted octupole vector (dimensionless) to account for
  // NC-ferrimagnets
