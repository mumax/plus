...
std::vector<unsigned int> VoronoiTessellator::generate(const Grid grid,
  ...
  grid_dims_ = real3{grid.size().x * cellsize.x, grid.size().y * cellsize.y,
  ...
  for (int nz = 0; nz < grid.size().z; nz++) {
    for (int ny = 0; ny < grid.size().y; ny++) {
      for (int nx = 0; nx < grid.size().x; nx++) {
        real3 coo = real3{nx * cellsize.x, ny * cellsize.y, nz * cellsize.z};
    ...
    for (int ty = t.pos.y - 1; ty <= t.pos.y + 1; ty++) {
      for (int tz = tz_start; tz <= tz_end; tz++) {
        ...
        std::vector<Center> centers =
            centersInTile(int3{wrapped_tx, wrapped_ty, wrapped_tz});
        for (auto c : centers) {
...
real3 VoronoiTessellator::periodicShift(const real3 coo, real3 center) {
  real3 d = center - coo;
  if (std::abs(d.x) > 0.5 * grid_dims_.x)
    center.x -= std::copysign(grid_dims_.x, d.x);
  if (std::abs(d.y) > 0.5 * grid_dims_.y)
    center.y -= std::copysign(grid_dims_.y, d.y);
  if (std::abs(d.z) > 0.5 * grid_dims_.z)
    center.z -= std::copysign(grid_dims_.z, d.z);
  return center;
}
