...
void wrap_window(py::module& m) {
    ...
    py::class_<Window>(m, "Window")
        .def(py::init<>())
        .def("setMagValue", [](Window& self, int side, real3 value) {
            self.setMagValue(static_cast<Boundary>(side), value);
        }, py::arg("side"), py::arg("value"))

        .def("setGeoValue", [](Window& self, int side, bool value) {
            self.setGeoValue(static_cast<Boundary>(side), value);
        }, py::arg("side"), py::arg("value"))

        .def("setRegValue", [](Window& self, int side, unsigned int value) {
